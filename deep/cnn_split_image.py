...
if test_saved_model:
    with tf.Session() as sess:
        # noise = None
        # saver.restore(sess, "./saved_models/split_image/noise_0_training_1000.ckpt")
        # print("Model restored.")
        # test_data = Data(noise, imageFiles='./data/noise_0/test_data/reshaped/*.png', labelFiles='./data/noise_0/test_data/reshaped/*.npy')
...
def train_diff_sim_noise_levels(noise):
    with tf.Session() as sess:
        print("Initialized session with simulation noise level {0}".format(noise))
    ...
    #     print("Model restored.")
        ...
        for step in range(0, training_iters, batch_size):     
            ...
            if step % 200000 == 0:
                ...
                val_data = Data(0, 200, imageFiles='./data/noise_{0}/val_data/reshaped/*.png'.format(noise), labelFiles='./data/noise_{0}/val_data/reshaped/*.npy'.format(noise))
                # val_data.x = normalize(val_data.x, normalization_mean, normalization_stdev)
                ...
                print("Validation Abs Loss={:.6f}".format(val_abs_error_mean))
                # print("Mean")
                save_path = saver.save(sess, "./saved_models/split_image/sim_noise_{0}.ckpt".format(noise))
                print("Saved model.")
...
# for training_size in [200, 400, 600]:
#     try:

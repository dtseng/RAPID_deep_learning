...
def train_diff_sim_noise_levels(noise):
    with tf.Session() as sess:
        print("Initialized session")
        data = Data(0, 1000, imageFiles="./data/noise_{0}/train_data/regular/*.png".format(noise), labelFiles="./data/noise_{0}/train_data/regular/*.npy".format(noise))
        # normalization_mean = np.mean(data.x, axis=0)
        # normalization_stdev = np.std(data.x, axis=0)
        # data.x = normalize(data.x, normalization_mean, normalization_stdev)
        # data.x = data.x/255.0
        ...
        for step in range(0, training_iters, batch_size):     
            ...
            if step % 1000 == 0:
                ...
                val_data = Data(0, 200, imageFiles="./data/noise_{0}/val_data/regular/*.png".format(noise), labelFiles="./data/noise_{0}/val_data/regular/*.npy".format(noise))
                # val_data.x = normalize(val_data.x, normalization_mean, normalization_stdev)
                # val_data.x = val_data.x/255.0
                ...
                save_path = saver.save(sess, "./saved_models/whole_image/sim_noise_{0}.ckpt".format(noise))
                print("Saved model.")
...
if test_saved_model:
    ...
    #     test_data = Data(noise, imageFiles='./data/noise_0/test_data/regular/*.png', labelFiles='./data/noise_0/test_data/regular/*.npy')
    #     test_batch = test_data.next_batch_no_shuffle(val_test_batch_size)
    #     test_x, test_y = next(test_batch)
    #     test_loss, test_abs_error_mean, test_abs_error_stdev = sess.run([cost, abs_error_mean, abs_error_stdev], feed_dict={x: test_x, y: test_y, keep_prob: 1.})
...
# for training_size in [200, 400, 600, 800]:
#     try:

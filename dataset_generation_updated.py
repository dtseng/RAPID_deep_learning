...
# Range of number of areas with high drainage rates.
MIN_NUM_GAUSSIANS = 2
MAX_NUM_GAUSSIANS = 4
...
# If true, each plant has indepent drainage uniformly at random between 0 and
# 1. Otherwise, dataset is generated by summing Gaussian areas of high drainage
# rate.
...
# and generates a vector of soil drainage rates for each of the 200 plants.
def soil_variation(X):
    # Choose the number of areas of high drainage uniformly at random in the
    # specified range.
    num_gaussians = np.random.randint(MIN_NUM_GAUSSIANS, MAX_NUM_GAUSSIANS + 1)

    # Vector of soil drainage at each of the plants.
    drainage = np.zeros(200)

    # Model each area of high drainage as a scaled multivariate normal 
    # distribution.
    for _ in range(num_gaussians):
        ...
        # soil drainage rates.
        drainage += 0.4 * (np.exp(-((X[0] - mu_x) ** 2) / (2 * (sigma_x ** 2))
                    - ((X[1] - mu_y) ** 2) / (2 * (sigma_y ** 2))))

    # Normalize drainage rates so that they are between 0 and 1.
    return np.clip(drainage, 0.0, 1.0)

# Class for individual plants in the vineyard.
class Plant(object):
    def __init__(self, position, init_soil_moisture):
        # Set soil drainage rate of this plant independently of other plants if
        # in potted plants mode.
        if potted_plants_mode:
            self.drainage_rate = np.random.uniform(0, 1)
    ...
    # Simulate plant growth using the supplied irrigation rate and drainage rate.
    def grow(self, irrigation_rate, drainage_rate):
        self.soil_moisture += irrigation_rate - drainage_rate
...
class Vineyard(object):
    def __init__(self):
        ...
        # Set local soil drainage rate for each individual plant.
        ...
        sc = self.ax2.imshow(np.flipud(dr), cmap=plt.get_cmap('RdBu_r'), extent=(0, self.bounds[0][1], 0, self.bounds[1][1]), interpolation='gaussian')
        # Legend.
    ...
    def update(self,i):
        ...
        for ind,vine in enumerate(self.vines):
            if potted_plants_mode:
                vine.grow(self.irrigation_rate[ind], vine.drainage_rate)
            else:
...
# NUM_TEST = 200
# NUM_UPDATES = 10
# TRAIN_DR_FILE1 = "new_data/train_data/reshaped/drain_rate{0}.npy"
# TRAIN_DR_FILE2 = "new_data/train_data/regular/drain_rate{0}.npy"
# TRAIN_IMAGE_FILE1 = "new_data/train_data/reshaped/image{0}.png"
# TRAIN_IMAGE_FILE2 = "new_data/train_data/regular/image{0}.png"
# VAL_DR_FILE1 = "new_data/val_data/reshaped/drain_rate{0}.npy"
# VAL_DR_FILE2 = "new_data/val_data/regular/drain_rate{0}.npy"
# VAL_IMAGE_FILE1 = "new_data/val_data/reshaped/image{0}.png"
# VAL_IMAGE_FILE2 = "new_data/val_data/regular/image{0}.png"
# TEST_DR_FILE1 = "new_data/test_data/reshaped/drain_rate{0}.npy"
# TEST_DR_FILE2 = "new_data/test_data/regular/drain_rate{0}.npy"
# TEST_IMAGE_FILE1 = "new_data/test_data/reshaped/image{0}.png"
# TEST_IMAGE_FILE2 = "new_data/test_data/regular/image{0}.png"

# for i in range(NUM_TRAIN):
#     vy = Vineyard()

#     np.save(TRAIN_DR_FILE1.format(i), vy.drainage_rate)
#     np.save(TRAIN_DR_FILE2.format(i), vy.drainage_rate)
...
#     np.save(TEST_DR_FILE1.format(i), vy.drainage_rate)
#     np.save(TEST_DR_FILE2.format(i), vy.drainage_rate)
